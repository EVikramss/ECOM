...
function errorSearch(props) {
    ...
    const [formData, setFormData] = useState({ errorKey: '', service: '', fromDate: '', toDate: '', sortOrder: 0, sortByField : 0 });
    const [errorVal, setErrorVal] = useState({strVal : ''});
	const [serviceNameList, setServiceNameList] = useState([]);
    const navigate = useNavigate();

    const searchError = async (e) => {
        e.preventDefault();
		let openErrorIntent = isUserIntentToOpenError(formData);
		navigate('/errorList', { state: { openSingleError : openErrorIntent, formData : formData} });
    }
	
	const isUserIntentToOpenError = (data) => {
		if(isValidString(data.errorKey)) {
			return true;
		} else {
			return false;
		}
	...
	useEffect(() => {
		try {
            ...
            axios.get(config.apiURL + "/getErrorServiceNames", {
                headers: {
                    Authorization: token
                }
            }).then(result => {
    ...
    return (
        <div className="headerSpacing">
            <Form className="container mt-4" onSubmit={searchError}>
                ...
                <div className="row align-items-end">
                    <div className="col-md-3">
                        <label className="form-label">Error Key</label>
                        <input type="text" placeholder="Enter Error Key" className="form-control" name="errorKey" onChange={onChangeField}
                        />
                    </div>
                </div>
				...
				<div className="row align-items-end">
					...
					<div className="d-flex gap-4">
						...
						<div className="d-flex gap-0 rounded border px-3 py-2">
							<div className="form-check form-check-inline">
								<input type="radio" checked={formData.sortByField === 0} className="form-check-input" id="errorDate" name="sortByField" value="0" onChange={onChangeFieldInt} />
								<label className="form-check-label" htmlFor="Asc">Error Date</label>
							</div>
							
							<div className="form-check form-check-inline">
								<input type="radio" checked={formData.sortByField === 1} className="form-check-input" id="service" name="sortByField" value="1" onChange={onChangeFieldInt} />
								<label className="form-check-label" htmlFor="Dsc">Service</label>
							</div>
						</div>
					</div>
				</div>
